...
func BenchmarkStringsReplace(b *testing.B) {
	for i := 0; i < b.N; i++ {
		s := "{a}:{b}:{c}:{d}:{e}"
		s = strings.ReplaceAll(s, "{a}", testData[0])
		s = strings.ReplaceAll(s, "{b}", testData[1])
		s = strings.ReplaceAll(s, "{c}", testData[2])
		s = strings.ReplaceAll(s, "{d}", testData[3])
		s = strings.ReplaceAll(s, "{e}", testData[4])
		_ = s
	}
}
